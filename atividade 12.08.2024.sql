...
INSERT INTO itens_pedido VALUES (null,1,23,4,9.50,38.00);
...
select cliente.nome,cliente.email,cidade.nome from cliente inner join cidade on cliente.idcidade = cidade.id; 
...
select cliente.nome,cliente.email,cidade.nome,estado.nome from cidade inner join cliente on cidade.id = cliente.idcidade inner join estado on estado.id = cliente.idestado where estado.nome = 'SP';
...
select cliente.nome,cliente.email,cliente.idCidade,cliente.idEstado from cliente  where nome like '%o%';

-- 4 - Listar o nome do produto e nome da marca dos produtos que custem entre 1000 e 3000 reais.

select * from produto;

select idMarca,preco  from produto where preco >1000 and preco < 3000;

-- 5 - Listar o nome do produto, nome da marca e nome da categoria dos produtos que tenham a letra "e" no nome do produto.

select  produto.nome,idMarca,idCategoria from produto where nome like '%e%';
